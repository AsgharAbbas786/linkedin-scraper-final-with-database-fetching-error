/*
  # Complete Authentication Database Schema Fix

  This migration creates a complete, working authentication system that integrates
  Supabase Auth with custom user profiles. It addresses the "Database error updating user"
  issue by ensuring all components work together properly.

  ## What this migration does:
  1. Creates all necessary tables with proper structure
  2. Sets up secure RLS policies
  3. Creates a robust user creation trigger
  4. Adds helper functions for user management
  5. Ensures proper permissions and indexes

  ## Tables Created:
  - `users`: Custom user profiles linked to auth.users
  - `apify_keys`: API key management for users
...
BEGIN
    ...
    IF _username IS NULL OR _username = '' THEN
        _username := split_part(_email, '@', 1) || '_' || substring(NEW.id::text from 1 for 8);
    END IF;
    ...
    IF _full_name IS NULL OR _full_name = '' THEN
        ...
        ELSE
            _full_name := split_part(_email, '@', 1);
        END IF;
    END IF;

    -- Insert user profile with conflict resolution
    ...
    ON CONFLICT (username) DO UPDATE SET
        username = EXCLUDED.username || '_' || extract(epoch from now())::text,
        updated_at = now()
    ...
    IF user_profile IS NULL THEN
        ...
        VALUES (
            ...
            COALESCE(
                ...
                split_part(auth_user.email, '@', 1) || '_' || substring(auth_user.id::text from 1 for 8)
            ),
            auth_user.email,
            COALESCE(
